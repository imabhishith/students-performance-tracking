...
// Global variable to track if difficulty is currently shown
let isDifficultyShown = false;

// Calculate exam difficulty based on class performance
function calculateExamDifficulty() {
    const examTotals = {};
    
    students.forEach(student => {
        student.exams.forEach(exam => {
            if (!examTotals[exam.exam]) {
                examTotals[exam.exam] = { 
                    totalScore: 0, 
                    totalMax: 0, 
                    count: 0,
            ...
            // Only count students who actually took the exam (maxTotal > 0)
            if (exam.maxTotal > 0) {
                examTotals[exam.exam].totalScore += exam.total;
                examTotals[exam.exam].totalMax += exam.maxTotal;
                examTotals[exam.exam].participatedStudents++;
                examTotals[exam.exam].scores.push(exam.total / exam.maxTotal * 100);
            }
            examTotals[exam.exam].count++;
        });
    });

    const difficultyScores = {};
    for (const examName in examTotals) {
        const data = examTotals[examName];
        if (data.participatedStudents > 0 && data.totalMax > 0) {
            const avgPercent = (data.totalScore / data.totalMax) * 100;
            const participationRate = data.participatedStudents / data.count;
            
            // Calculate standard deviation for more sophisticated analysis
            const scores = data.scores;
            const mean = avgPercent;
            const variance = scores.reduce((acc, score) => acc + Math.pow(score - mean, 2), 0) / scores.length;
            const stdDev = Math.sqrt(variance);
            
            // Base difficulty: 10 - (avgPercent / 10) maps 0-100% to 10-1
            let difficulty = 10 - (avgPercent / 10);
            
            // Adjust for low participation (could indicate difficulty)
            if (participationRate < 0.8) {
                difficulty += (1 - participationRate) * 2;
            }
            
            // Adjust for high standard deviation (inconsistent performance can indicate difficulty)
            if (stdDev > 20) {
                difficulty += (stdDev - 20) / 10;
            }
            
            // Clamp between 1-10 and round to 1 decimal
            difficulty = Math.min(10, Math.max(1, difficulty));
            
            difficultyScores[examName] = {
                score: parseFloat(difficulty.toFixed(1)),
                avgPercent: parseFloat(avgPercent.toFixed(2)),
                participation: Math.round(participationRate * 100),
                stdDev: parseFloat(stdDev.toFixed(2)),
                participated: data.participatedStudents,
                total: data.count
            };
        } else {
            difficultyScores[examName] = {
...
function getDifficultyLevel(score) {
    if (score === 'N/A') return { text: 'N/A', color: '#95a5a6' };
    
    if (score <= 2) return { text: 'Very Easy', color: '#27ae60' };
    else if (score <= 4) return { text: 'Easy', color: '#2ecc71' };
    else if (score <= 6) return { text: 'Moderate', color: '#f39c12' };
    else if (score <= 8) return { text: 'Hard', color: '#e67e22' };
    else return { text: 'Very Hard', color: '#e74c3c' };
}

// Show exam difficulty in a modal
function showExamDifficultyModal() {
    const difficultyScores = calculateExamDifficulty();
    
    // Sort exams by natural order (WE 1, WE 2, WE 3, RT 1, etc.)
    const examOrder = ['WE 1', 'WE 2', 'WE 3', 'RT 1', 'RT 2', 'RT 3'];
    const sortedExams = Object.keys(difficultyScores).sort((a, b) => {
        const aIndex = examOrder.indexOf(a);
        const bIndex = examOrder.indexOf(b);
    ...
    const modalHTML = `
        <div id="difficultyModal" class="modal" style="
            display: block;
            position: fixed;
            z-index: 1000;
            left: 0;
        ...
        ">
            <style>
                ...
                .difficulty-tooltip:hover::after {
            ...
            <div class="modal-content difficulty-modal-content" style="
            ...
            ">
                ...
                ">
                    <div>
                        <h2 style="margin: 0; font-size: 24px;">ðŸ“Š Exam Difficulty Analysis</h2>
                        <p style="margin: 5px 0 0 0; opacity: 0.9; font-size: 14px;">Advanced Performance Analytics</p>
                    </div>
                ...
                <div style="padding: 20px; max-height: calc(90vh - 140px); overflow-y: auto;">
                    ...
                    ">
                        <h4 style="margin: 0 0 10px 0; color: #2c3e50;">ðŸ“‹ How Difficulty is Calculated</h4>
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; font-size: 13px; color: #555;">
                            <div><strong>ðŸ“ˆ Average Performance:</strong> Lower class average = Higher difficulty</div>
                            <div><strong>ðŸ‘¥ Participation Rate:</strong> Low participation can indicate high difficulty</div>
                            <div><strong>ðŸ“Š Score Distribution:</strong> High variation suggests challenging exam</div>
                            <div><strong>ðŸŽ¯ Scale:</strong> 1 (Very Easy) â†’ 10 (Very Hard)</div>
                        </div>
                    </div>
                        ...
                        <table style="width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 14px;">
                            <thead>
                                <tr style="background: linear-gradient(135deg, #3498db 0%, #2980b9 100%); color: white;">
                                    <th style="padding: 15px 10px; text-align: left; border: none; font-weight: 600;">Exam</th>
                                    <th style="padding: 15px 10px; text-align: center; border: none; font-weight: 600;">
                                        <span class="difficulty-tooltip" data-tooltip="Difficulty score from 1-10">
                                            Difficulty Score
                                        </span>
                                    </th>
                                    <th style="padding: 15px 10px; text-align: center; border: none; font-weight: 600;">
                                        <span class="difficulty-tooltip" data-tooltip="Class average percentage">
                                            Class Avg %
                                        </span>
                                    </th>
                                    <th style="padding: 15px 10px; text-align: center; border: none; font-weight: 600;">
                                        <span class="difficulty-tooltip" data-tooltip="Students who attempted vs total">
                                            Participation
                                        </span>
                                    </th>
                                    <th style="padding: 15px 10px; text-align: center; border: none; font-weight: 600;">
                                        <span class="difficulty-tooltip" data-tooltip="Score variation among students">
                                            Std Dev
                                        </span>
                                    </th>
                                    <th style="padding: 15px 10px; text-align: center; border: none; font-weight: 600;">Difficulty Level</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${sortedExams.map((exam, index) => {
                                    const data = difficultyScores[exam];
                                    const level = getDifficultyLevel(data.score);
                                    ...
                                    return `
                                        ...
                                        " onmouseover="this.style.backgroundColor='#e3f2fd'" onmouseout="this.style.backgroundColor='${rowBg}'">
                                            <td style="padding: 15px 10px; font-weight: 600; color: #2c3e50;">
                                                ${exam}
                                            </td>
                                            ...
                                            <td style="padding: 15px 10px; text-align: center;">
                                                <span style="font-weight: 500;">${data.avgPercent}%</span>
                                            </td>
                                            <td style="padding: 15px 10px; text-align: center;">
                                                <div style="font-weight: 500;">${data.participation}%</div>
                                                <div style="font-size: 11px; color: #666;">
                                                    (${data.participated}/${data.total})
                                                </div>
                                            </td>
                                            <td style="padding: 15px 10px; text-align: center;">
                                                <span style="font-weight: 500;">${data.stdDev}</span>
                                            </td>
                                            <td style="padding: 15px 10px; text-align: center;">
                                                <span style="
                    ...
                    <div style="margin-top: 25px; text-align: center;">
                        <button id="closeDifficultyModal" style="
    ...
    const closeButton = document.getElementById('closeDifficultyModal');
...
// Add the Exam Difficulty button to the UI
function addExamDifficultyButton() {
    // Try to find existing button first
    if (document.getElementById('btnExamDifficulty')) {
        return; // Button already exists
    }
    
    // Look for tab containers or control areas
    const possibleContainers = [
        ...
        // Look for any div that might contain buttons
    ...
    if (targetContainer) {
        ...
        difficultyBtn.textContent = 'ðŸ“Š Exam Difficulty';
        difficultyBtn.id = 'btnExamDifficulty';
        ...
        difficultyBtn.addEventListener('mouseenter', () => {
            difficultyBtn.style.transform = 'translateY(-2px)';
            difficultyBtn.style.boxShadow = '0 4px 15px rgba(155, 89, 182, 0.4)';
            difficultyBtn.style.background = 'linear-gradient(135deg, #8e44ad 0%, #732d91 100%)';
        });
        
        difficultyBtn.addEventListener('mouseleave', () => {
            difficultyBtn.style.transform = 'translateY(0)';
            difficultyBtn.style.boxShadow = '0 2px 8px rgba(155, 89, 182, 0.3)';
            difficultyBtn.style.background = 'linear-gradient(135deg, #9b59b6 0%, #8e44ad 100%)';
        });
        
        // Add click event
        difficultyBtn.addEventListener('click', (e) => {
            e.preventDefault();
            showExamDifficultyModal();
        });
        
        // Insert button (try to place it logically)
        if (targetContainer.querySelector('button')) {
            // If there are other buttons, add after them
            const lastButton = Array.from(targetContainer.querySelectorAll('button')).pop();
            lastButton.parentNode.insertBefore(difficultyBtn, lastButton.nextSibling);
        } else {
            // Otherwise add at the beginning
            targetContainer.insertBefore(difficultyBtn, targetContainer.firstChild);
        }
        
        console.log('âœ… Exam Difficulty button added successfully');
    } else {
        // Fallback: create a floating button
        console.warn('âš ï¸ Tab container not found, adding floating button');
        const difficultyBtn = document.createElement('button');
        difficultyBtn.textContent = 'ðŸ“Š';
        difficultyBtn.id = 'btnExamDifficulty';
        difficultyBtn.title = 'Exam Difficulty Analysis';
        difficultyBtn.style.cssText = `
            position: fixed;
            top: 20px;
            right: 20px;
        ...
        difficultyBtn.addEventListener('mouseenter', () => {
            ...
            difficultyBtn.style.boxShadow = '0 6px 25px rgba(155, 89, 182, 0.5)';
        });
        
        difficultyBtn.addEventListener('mouseleave', () => {
            difficultyBtn.style.transform = 'scale(1)';
            difficultyBtn.style.boxShadow = '0 4px 20px rgba(155, 89, 182, 0.4)';
        });
        ...
        document.body.appendChild(difficultyBtn);
    }
}

// Initialize the exam difficulty feature
function initializeExamDifficulty() {
    // Add button after a short delay to ensure DOM is ready
    setTimeout(() => {
        addExamDifficultyButton();
    }, 1000);
}
...
function populateStats() {
    ...
    allExams.localcompare().forEach(exam => {
